...
const LeavesForFacultyPage = () => {
    ...
    const [absenceDateByStudent, setAbsenceDateByStudent] = useState({});
    ...
    useEffect(() => {
        ...
        const fetchProjects = async () => {
            try {
                if (user?.id) {
                    const resp = await axios.get(`/api/v1/project/${user.id}`);
    ...
    const handleBatchMarkAbsence = async () => {
        try {
            if (!batchDate) {
            ...
            if (selectedProjectId) {
                await axios.post('/api/v1/leaves/mark-absence', {
                    project_id: selectedProjectId,
                    date: batchDate,
                });
            } else {
                const user_ids = Object.entries(selectedStudents)
                    .filter(([_, v]) => v)
                    .map(([k]) => k);
                if (user_ids.length === 0) {
                    setError('Select at least one student or a project');
                    setLoading(false);
                    return;
                }
                await axios.post('/api/v1/leaves/mark-absence', {
            ...
            console.error('Batch absence error:', err);
    ...
    const handleMarkAbsence = async (studentId) => {
        try {
            const date = absenceDateByStudent[studentId];
            if (!date) return;
            setLoading(true);
            await axios.post('/api/v1/leaves/mark-absence', {
                user_id: studentId,
    ...
    return (
        <div className="leaves-content-container">
            ...
            <div className="leaves-controls">
                ...
                <div className="leaves-batch-controls">
                    <input
                        type="date"
                        value={batchDate}
                        onChange={(e) => setBatchDate(e.target.value)}
                        className="leaves-absence-date-input"
                        aria-label="Batch absence date"
                    />
                    <select
                        value={selectedProjectId}
                        onChange={(e) => setSelectedProjectId(e.target.value)}
                        className="leaves-project-select"
                    >
                        <option value="">Mark by project (optional)</option>
                        {projects.map(p => (
                            <option key={p._id} value={p._id}>{p.name}</option>
                        ))}
                    </select>
                    <button
                        className="leaves-mark-absence-btn"
                        onClick={handleBatchMarkAbsence}
                        disabled={!batchDate}
                    >
                            ...
                            {filteredGroupedLeaves.map(([studentId, group]) => (
                                group.leaves.map((leave, index) => (
                                    <tr key={leave._id} className="leaves-table-row">
                                        ...
                                        {index === 0 && (
                                            <td rowSpan={group.leaves.length} className="leaves-select-cell">
                                                <button
                                                    className="leaves-select-toggle"
                                                    onClick={() => {
                                                        const anySelected = group.leaves.some(() => selectedStudents[studentId]);
                                                        setSelectedStudents(prev => ({
                                                            ...prev,
                                                            [studentId]: !anySelected
                                                        }));
                                                    }}
                                                    title="Select student for batch"
                                                >
                                                    <FontAwesomeIcon icon={selectedStudents[studentId] ? faCheckSquare : faSquare} />
                                                </button>
                                            </td>
                                        )}
                                        ...
                                        {index === 0 && (
                                            <td rowSpan={group.leaves.length} className="leaves-absence-cell">
                                                <input
                                                    type="date"
                                                    value={absenceDateByStudent[studentId] || ''}
                                                    onChange={(e) => setAbsenceDateByStudent(prev => ({ ...prev, [studentId]: e.target.value }))}
                                                    className="leaves-absence-date-input"
                                                />
                                                <button
                                                    className="leaves-mark-absence-btn"
                                                    onClick={() => handleMarkAbsence(studentId)}
                                                    disabled={!absenceDateByStudent[studentId]}
                                                >
                                                    Mark Absent
                                                </button>
                                            </td>
                        ...
                        <div key={studentId} className="leaves-student-card">
                            ...
                            {expandedStudent === studentId && (
                                <div className="leaves-list">
                                    {/* Mark absence controls in card view */}
                                    <div className="leaves-card-absence-controls">
                                        <input
                                            type="date"
                                            value={absenceDateByStudent[studentId] || ''}
                                            onChange={(e) => setAbsenceDateByStudent(prev => ({ ...prev, [studentId]: e.target.value }))}
